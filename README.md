...
| **항목** | **설명** |
| --- | --- |
| **Spring Boot 프로젝트 설정 및 테스트** | Spring Boot 프로젝트를 설정하고, 이를 통해 간단한 테스트 페이지를 만들면서 Spring Boot의 기본 설정, MyBatis와 MariaDB의 연동, 그리고 간단한 컨트롤러 작성 방법을 배웠다. Spring Boot는 초기 설정만 잘 해두면 매우 빠르고 효율적으로 웹 애플리케이션을 개발할 수 있다는 점을 느꼈다. MyBatis와 MariaDB를 연동하는 과정에서 데이터베이스와의 연동이 얼마나 중요한지 깨달았다. |
| **MyBatis와 Spring Boot를 활용한 API 개발** | MyBatis를 이용해 데이터베이스와의 연동을 설정하고, 특정 연도의 로그인 수를 조회하는 API를 개발했다. 이 과정에서 SQL 쿼리를 MyBatis 매퍼 파일에서 정의하고, 이를 서비스와 컨트롤러에서 사용하는 방법을 익혔다. API를 개발하면서 SQL 쿼리를 작성하고, 이를 통해 원하는 데이터를 정확하게 가져오는 것이 중요하다는 것을 깨달았다. 데이터베이스와의 연동을 통해 서버와 클라이언트 간의 데이터 흐름을 이해하게 되었다. |
| **API 가이드 문서 작성** | API 문서를 작성할 때 필요한 주요 개념들(요청 URL, 요청 파라미터, 응답 바디 등)을 익히면서 API가 무엇인지, 이를 문서화하는 과정에서 어떤 요소들이 필요한지 이해했다. 또한, 작성한 API를 바탕으로 가이드 문서를 작성하며, 단순한 형식적 작업이 아닌 사용자가 API를 쉽게 이해하고 활용할 수 있도록 명확하게 설명하는 것이 중요함을 깨달았다. |
| **자바 코드로 비즈니스 로직 구현** | 단순한 SQL 쿼리로 해결할 수 없는 부분을 자바 코드를 조합하여 해결하는 방법을 배웠다. 예를 들어, 휴일을 제외한 로그인 수를 계산하는 과제를 수행했다. 단순한 쿼리로는 해결할 수 없는 복잡한 비즈니스 로직을 구현하는 과정에서 자바의 유연성을 느꼈다. 데이터베이스와의 연동뿐만 아니라, 실제 비즈니스 로직을 자바 코드로 구현하는 것이 얼마나 중요한지 깨달았다. |
...
| **항목** | **설명** |
| --- | --- |
| **개발 환경 세팅의 중요성** | 스프링 환경 세팅이 초심자에게 어려울 수 있지만, 실무에서 프로젝트를 진행할 때 가장 기본이 되는 작업임을 깨달았다. 다양한 버전의 소프트웨어와 JDK를 다루어야 하는 만큼, 환경 세팅은 프로젝트의 성공에 매우 중요한 역할을 한다는 점을 인식했다. |
| **버전 관리의 필요성** | 환경 설정 시 각 소프트웨어, JDK, Maven dependency 등의 버전을 정확하게 기록하는 것이 실무에서 중요한 이유를 이해했다. 이는 신규 인력이 들어오거나 버전 업그레이드 시 큰 도움이 된다는 점을 배웠다. |
| **실무에서의 환경 설정** | 실무에서는 Docker를 활용해 환경 설정을 자동화하거나, 기존 설치 디렉토리를 복사해 새로운 환경을 빠르게 구성할 수 있다는 것을 알게 되었다. 이러한 방법들이 실무에서 효율성을 높일 수 있다는 점을 이해했다. |
| **Maven dependency 관리** | Spring에서 Maven의 pom.xml 파일을 통해 쉽게 라이브러리를 관리할 수 있지만, 폐쇄망에서 작업할 때는 라이브러리를 직접 폴더에 넣어야 할 수도 있다는 점을 알게 되었다. 이럴 경우, 팀원들과 협력하여 문제를 해결하는 것이 중요하다는 것을 깨달았다. |
| **MVC 패턴과 MyBatis 활용** | URL로 들어오는 요청을 컨트롤러에서 받아, 서비스 레이어에서 데이터를 처리한 후 결과를 반환하는 것이 기본적인 MVC 패턴임을 이해했다. 이 과정에서 MyBatis를 사용해 효율적으로 SQL 쿼리를 작성하고, 필요한 경우 HashMap이나 List 형태로 데이터를 반환할 수 있음을 알게 되었다. |
