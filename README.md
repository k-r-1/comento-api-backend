...
- **[피드백을 통해 배운 교훈]**
  ...
  - **MVC 패턴과 MyBatis 활용:** URL로 들어오는 요청을 컨트롤러에서 받아, 서비스 레이어에서 데이터를 처리한 후 결과를 반환하는 것이 기본적인 MVC 패턴임을 이해했다. 이 과정에서 MyBatis를 사용해 효율적으로 SQL 쿼리를 작성하고, 필요한 경우 HashMap이나 List 형태로 데이터를 반환할 수 있음을 알게 되었다. 또한, VO(Value Object) 클래스를 사용해 명확하게 데이터를 다루는 방법을 이해했으며, 복잡한 쿼리의 경우 MyBatis를 통해 유연하게 처리할 수 있다는 점도 배웠다.
<br></br>
